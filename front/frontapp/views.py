...
def action_get_offers(request):
    if request.is_ajax and request.method == "GET":
        response = requests.get('http://host.docker.internal:8002/api/get_offers', params=request.GET)
        return JsonResponse(response.json(), status=response.status_code)
    return JsonResponse({'error': 'wrong method'}, status=405)
...
def action_add_offer(request):
    if request.is_ajax and request.method == "POST":
        response = requests.post('http://host.docker.internal:8002/api/add_offer', data=request.POST)
        return JsonResponse(response.json(), status=response.status_code)
    return JsonResponse({'error': 'wrong method'}, status=405)
...
def home_view(request):
    ...
    response = requests.get('http://host.docker.internal:8002/api/get_offers')
...
def offer_detail_view(request, id):
    context = get_context_from_session(request)
    response = requests.get(f'http://host.docker.internal:8002/api/get_offer/{id}')
